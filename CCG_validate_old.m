function [ obj_first_stage, obj_second_stage, y_optimal, x_feasible, u_feasible ] = CCG_validate( mat_A, mat_B, mat_C, mat_D, mat_F, mat_G, vec_b, vec_c, vec_d, vec_e, vec_f, vec_g, vec_h, vec_u0, vec_du_pos, vec_du_neg, x_star, u_star)
...
if prod(mat_F*u_star <= vec_f+eps) && prod(mat_G*(zpos+zneg) <= vec_h+eps)
    u_feasible = 1;
else
    u_feasible = 0;
end
...
nVar = size(mat_D,2); % # second-stage variables
nCon = size(mat_D,1);
...
if ( strcmp(result.status,'OPTIMAL') )
    ...
    obj_first_stage = 0; obj_second_stage = 0; y_optimal = NaN*ones(nYvar, 1);
end
