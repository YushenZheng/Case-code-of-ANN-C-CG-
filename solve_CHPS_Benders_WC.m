...
MAX_ITER = 50;
tol = 1e-6;
time_master = zeros(MAX_ITER, 1);
time_network = zeros(MAX_ITER, nPrd);
time_dhn = zeros(MAX_ITER, 1);
...
model_first.objcon = sum(sum(mpc.windpen*ub(index_var_pg(index_wind2gen(:),:))));
...
while ( (bFlag_network==false | bFlag_DHS==false) &  nCount_network <= MAX_ITER)
    bFlag_network = true;
    ...
    time_master(nCount_master) = result_master.runtime;
    ...
    for t=1:nPrd
        tic;
        [coef_temp, const_temp, bFeas] = feasibility_check_WC( ...
            ...
            ub(index_var_a(:,t)), ... % ub
            x_master, 1e-6);% x
        time_network(nCount_network, t) = toc;
...
%     end    
    ...
    time_temp = zeros(2*nChpplant*nPrd,1);
    for s=1:2*nChpplant*nPrd
        tic;
        [coef_temp, const_temp, bFeas] = feasibility_check_WC( ...
    ...
    time_dhn(nCount_DHS) = max(time_temp);
...
if ( bFlag_network==true && bFlag_DHS==true)
    ...
    time_benders = sum(time_master) + sum(max(time_network,[],2)) + sum(time_dhn);
    save benders.mat
else
    err('solve_CHPS_Benders failed');
end